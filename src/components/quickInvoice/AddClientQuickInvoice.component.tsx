...
import { setClientQuickInvoice } from "@myCash/lib";
...
export const AddClientQuickInvoice: React.FC<AddClientProps> = ({
  t,
  cartState,
  handleOpen,
  open,
  ...
  const handleSubmit = (
    ...
    if (values?.id) {
      dispatch(setClientQuickInvoice(values));
      handleClose();
      return;
    }
    mutate(values, {
      onSuccess: (res) => {
        if (res?.data?.status !== 1) {
          ...
          dispatch(setClientQuickInvoice(res?.data?.data));
          actions.resetForm();
          handleClose();
        }
      },
