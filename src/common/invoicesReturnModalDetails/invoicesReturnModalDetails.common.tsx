...
type invoicesReturnModalDetailsProps = {
  open: boolean;
  handleClose: () => void;
  invoice: InvoiceType;
};

export const InvoicesReturnModalDetails: React.FC<invoicesReturnModalDetailsProps> = ({
  open,
  handleClose,
  invoice,
}) => {
...
//   const invoiceCashID = invoice.id?invoice.id:localStorage.getItem("invoiceCashID") ;
//   const { data, isPending,refetch } = useGetInvoice(`${invoiceCashID}` || "");
// console.log(data?.data?.data);

  useEffect(() => {if(invoice){dispatch(setReturnInvoice(invoice));}
  ...
  return (
    ...
    >
      <Stack gap={"12px"}>
        <ReturnInvoiceInfo invoice={returnInvoiceState.invoice||invoice} />
      </Stack>
    </ModalInvoice>
  );
};
